...
async function run() {
  try {
    ...
    app.patch("/contests/:id", async (req, res) => {
      ...
      const result = await contestsCollection.updateOne({ _id: new ObjectId(id) }, { $set: updateData });
      res.send(result);
    });
    ...
    app.patch("/payment-success", async (req, res) => {
      try {
        ...
        await paymentsCollection.insertOne({ transactionId, trackingId, contestId, email, amount, createdAt: new Date() });
        await contestsCollection.updateOne({ _id: new ObjectId(contestId) }, { $set: { paymentStatus: "paid", transactionId, trackingId } });
    ...
    app.get("/payments", verifyFBToken, async (req, res) => {
      ...
      if (!email || email !== req.decoded_email) return res.status(403).send({ message: "Forbidden access" });
    ...
    app.patch("/creator/:id", verifyFBToken, async (req, res) => {
      ...
      await creatorCollection.updateOne({ _id: new ObjectId(id) }, { $set: { status } });

      if (status === "approved" && email) {
        await userCollection.updateOne({ email }, { $set: { role: "creator" } });
      }

      res.send({ message: "Creator status updated" });
    });
